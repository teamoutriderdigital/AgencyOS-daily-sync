...
function RockCard({ rock, summaries }: { rock: Rock; summaries: Map<string, ItemSummary> }) {
  ...
  const progress = progressFraction(rock);
  return (
    <div className="rounded-lg border border-border bg-surface-alt/40 px-3 py-2">
      ...
      {progress !== null && (
        <div className="mt-1.5 flex items-center gap-2">
          <div className="h-1.5 flex-1 overflow-hidden rounded-full border border-border bg-surface">
            <div
              className={cn("h-full rounded-full transition-all", progress === 1 ? "bg-blue-500" : "bg-green-500")}
